...
export default function Widget({ greeting = 'Hello' }: { greeting?: string }) {
  ...
  return (
    <div className="remote-widget remote-widget-dtd">
      <h2>Digital Training Delivery</h2>
      <p>
        {greeting} 👋 — this card is rendered by the <strong>dtd</strong> micro-frontend.
      </p>
      <button onClick={() => setCount((c) => c + 1)}>DTD count: {count}</button>
      <p style={{ fontFamily: 'ui-monospace, monospace', fontSize: 12, color: '#888' }}>
        dtd widget version: {import.meta.env.VITE_APP_VERSION ?? 'dev'}
      </p>
    </div>
  )
}
